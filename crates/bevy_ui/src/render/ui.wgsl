...
fn distance_round_border(position: vec2<f32>, size: vec2<f32>, radius_vec: vec4<f32>) -> f32 {
    var i = select(0, 1, position.y > 0.0) + select(0, 2, position.x > 0.0);
    var radius = radius_vec[i];

    var q = abs(position) - size + radius;
...
fn fragment(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    // this makes 1-2px borders look good while making bigger ones smoother.
    var edge_softness = clamp(in.border_width - 1.0, 0.0, 2.0);
    var border_softness = clamp(in.border_width - 1.0, 0.0, 1.0);
    var new_radius  = vec4<f32>(0.0, 25.0, 0.0, 25.0);
